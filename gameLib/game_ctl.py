...
class GameControl():
    ...
    def window_full_shot(self, file_name=None, gray=0):
        ...
        try:
            if (not hasattr(self, 'memdc')):
                self.init_mem()
            if self.client == 0:
                self.memdc.BitBlt((0, 0), (self._client_w, self._client_h), self.srcdc, (self._border_l, self._border_t), win32con.SRCCOPY)
            else:
                self.memdc.BitBlt((0, -35), (self._client_w, self._client_h), self.srcdc, (self._border_l, self._border_t), win32con.SRCCOPY)
            if file_name != None:
    ...
    def window_part_shot(self, pos1, pos2, file_name=None, gray=0):
        ...
        if self.client == 0:
            memdc.BitBlt((0, 0), (w, h), srcdc, (pos1[0]+self._border_l, pos1[1]+self._border_t), win32con.SRCCOPY)
        else:
            memdc.BitBlt((0, -35), (w, h), srcdc, (pos1[0]+self._border_l, pos1[1]+self._border_t), win32con.SRCCOPY)
        if file_name != None:
